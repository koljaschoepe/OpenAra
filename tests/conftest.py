...
from arasul_tui.core.platform import GpuInfo, Platform, StorageInfo, reset_platform
...
@pytest.fixture(autouse=True)
def _reset_platform():
    """Clear platform singleton between tests to prevent state leakage."""
    reset_platform()
    yield
    reset_platform()
...
# ---------------------------------------------------------------------------
# Shared Platform factories & helpers
# ---------------------------------------------------------------------------


def mock_platform(platform: Platform):
    """Context manager to mock get_platform() — use in ``with`` blocks.

    Example::

        with mock_platform(jetson_platform):
            result = cmd_status(state, [])
    """
    return patch("arasul_tui.core.platform.get_platform", return_value=platform)
