...
def test_show_welcome(mock_platform):
...
def test_ask_name_skip(mock_mark):
    result = _onboarding_ask_name(_state(), "skip")
    assert result.ok is True
    assert result.refresh is True
    mock_mark.assert_called_once()


def test_ask_name_enter():
    """Pressing Enter proceeds to name prompt."""
    result = _onboarding_ask_name(_state(), "")
    assert result.ok is True
    assert result.prompt == "Your first name: "
...
@patch("arasul_tui.core.onboarding._show_step2_claude")
def test_save_name_skip(mock_step2):
    from arasul_tui.core.types import CommandResult

    mock_step2.return_value = CommandResult(ok=True, style="silent")
    _onboarding_save_name(_state(), "skip")
    mock_step2.assert_called_once()
