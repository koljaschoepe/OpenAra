...
def _git_wizard_auth_token(state: TuiState, user_input: str) -> CommandResult:
    ...
    if result.returncode != 0:
        ...
        # Strip any accidental token echo from error output
        if token in err:
...
def cmd_git(state: TuiState, args: list[str]) -> CommandResult:
    ...
    if sub in ("pull", "push", "log", "status") and not state.active_project:
        ...
        print_info("Open one first: [bold]/open <name>[/bold]")
        return CommandResult(ok=False, style="silent")

    # Verify project is a git repository
    if (
        sub in ("pull", "push", "log", "status")
        and state.active_project
        and not (state.active_project / ".git").exists()
    ):
        print_error("Not a git repository.")
        print_info("Initialize with: [bold]git init[/bold]")
        return CommandResult(ok=False, style="silent")
    ...
    if sub == "pull":
        ...
        if "Already up to date" in output:
            ...
            return CommandResult(ok=False, style="silent")
        elif "CONFLICT" in output or "Automatic merge failed" in output:
            print_error("Merge conflict detected. Resolve conflicts manually:")
            print_info("  [bold]git status[/bold]  — see conflicted files")
            print_info("  [bold]git merge --abort[/bold]  — undo the merge")
            if output:
                console.print(f"{content_pad()}[dim]{output[:300]}[/dim]", highlight=False)
            return CommandResult(ok=False, style="silent")
        else:
    ...
    if sub == "push":
        ...
        if "error" in output.lower() or "fatal" in output.lower():
            print_error(output[:200])
            if "no upstream" in output.lower() or "no configured push" in output.lower():
                print_info("Try: [bold]git push -u origin main[/bold]")
            elif "rejected" in output.lower():
                print_info("Pull first: [bold]/git pull[/bold]")
            return CommandResult(ok=False, style="silent")
        else:
